/**
 * FluidFlowBackground — extracted from Framer Marketplace
 * Source: https://framerusercontent.com/modules/8OxUMx4ok0KwgVuRcOCs/fDmF21fbHJGL1qyHn3oQ/FluidFlowBackground.js
 *
 * Usage:
 *   <FluidFlowBackground colors={["#5227FF","#FF9FFC","#B19EEF"]} />
 *
 * Props:
 *   colors           string[]  Palette array (2–5 hex colors)
 *   mouseForce       number    Push intensity (default 20)
 *   cursorSize       number    Interaction radius (default 100)
 *   isViscous        boolean   Honey-like flow (default false)
 *   viscous          number    Viscosity amount (default 30)
 *   resolution       number    0.1–1.0, lower = faster (default 0.25)
 *   isBounce         boolean   Bounce at edges (default false)
 *   autoDemo         boolean   Auto-wander when idle (default true)
 *   autoSpeed        number    Auto-wander speed (default 0.5)
 *   autoIntensity    number    Auto-wander strength (default 2.2)
 *   style            object    Extra inline styles for the container
...
// Stub out Framer's addPropertyControls — not needed outside Framer
...
export default function FluidFlowBackground({
  ...
  useEffect(() => {
    ...
    const line_vert = `
      attribute vec3 position; uniform vec2 px; precision highp float;
      varying vec2 uv;
    ...
    const advection_frag = `
      ...
      void main(){
        ...
        if(isBFECC==false){
          ...
          gl_FragColor=vec4(texture2D(velocity,spot_new3-vel_2*dt*ratio).xy,0.0,0.0);
        }
      }`;
    ...
    class MouseClass {
      constructor() {
        ...
        this.isAutoActive = false; this.autoIntensity = 2;
    ...
    // ── ShaderPass & simulation classes (verbatim from source) ────────────────
    class ShaderPass {
    ...
    class Advection extends ShaderPass {
      constructor(simProps) {
        ...
        this.line = null; this.uniforms = this.props.material.uniforms; this.init();
      }
      init() {
        super.init();
    ...
    class ExternalForce extends ShaderPass {
      constructor(simProps) { super({ output: simProps.dst }); this.mouse = null; this._simProps = simProps; this.init(simProps); }
      init(simProps) {
        super.init();
      ...
      update(props = {}) {
        const fX = Mouse.diff.x / 2 * (props.mouse_force || 0);
        const fY = Mouse.diff.y / 2 * (props.mouse_force || 0);
        const cs = props.cellScale || { x: 1, y: 1 };
        const cSize = props.cursor_size || 0;
        const cX = Math.min(Math.max(Mouse.coords.x, -1 + cSize * cs.x + cs.x * 2), 1 - cSize * cs.x - cs.x * 2);
        const cY = Math.min(Math.max(Mouse.coords.y, -1 + cSize * cs.y + cs.y * 2), 1 - cSize * cs.y - cs.y * 2);
        const u = this.mouse.material.uniforms;
        u.force.value.set(fX, fY); u.center.value.set(cX, cY); u.scale.value.set(cSize, cSize);
        super.update();
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(args = {}) { if (this.uniforms && args.vel) this.uniforms.velocity.value = args.vel.texture; super.update(); }
    }
    ...
    class Pressure extends ShaderPass {
      ...
      update(args = {}) { if (this.uniforms && args.vel && args.pressure) { this.uniforms.velocity.value = args.vel.texture; this.uniforms.pressure.value = args.pressure.texture; } super.update(); }
    }
    ...
    class Simulation {
      ...
      calcSize() {
        const w = Math.min(Math.max(1, Math.round(this.options.resolution * Common.width)), 512);
        const h = Math.min(Math.max(1, Math.round(this.options.resolution * Common.height)), 512);
  ...
  return _jsx("div", {
    ref: mountRef,
    className: `fluid-flow-bg ${className || ""}`,
    style: style,
    children: _jsx("style", {
      children: `.fluid-flow-bg { position: relative; overflow: hidden; width: 100%; height: 100%; touch-action: none; }`
    })
  });
}
